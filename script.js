...
function bindEls() {
  return {
    ...
    refView: document.getElementById("ref-view"),
    refTitle: document.getElementById("ref-title"),
    refMeta: document.getElementById("ref-meta"),
    refFrame: document.getElementById("ref-frame"),
    refBackBtn: document.getElementById("ref-back-btn"),
  };
}

function boot() {
  els.appTitle.textContent = APP_TITLE;
  els.appSubtitle.textContent = "Quick, offline-first clinical reference for UC night float.";
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      if (!els.refView.classList.contains("hidden") || !els.sectionView.classList.contains("hidden")) {
        goBack();
      } else {
        els.searchInput.value = "";
        renderHome("");
...
function setDocTitle(suffix) {
  document.title = suffix ? `${suffix} • ${APP_TITLE}` : APP_TITLE;
}
...
function buildSearchIndex() {
  searchIndex = SECTIONS.map((s) => {
    ...
    const raw = String(s.html || "").toLowerCase();

    // Strip tags for full-text search
    const text = raw.replace(/<[^>]+>/g, " ");
    const haystack = `${title} ${text}`.replace(/\s+/g, " ").trim();
...
function pushCurrentView() {
  if (!els) return;
  if (!els.refView.classList.contains("hidden") && currentRefFile) {
    navStack.push({ view: "ref", refFile: currentRefFile });
    return;
  }
  if (!els.sectionView.classList.contains("hidden") && typeof currentSectionIndex === "number") {
...
function showReference(refFile, pushState = true) {
  ...
  // Remember section index for reliable return (e.g., Adults w/ complexity one-pagers, insulin pharmacokinetics)
  if (!els.sectionView.classList.contains("hidden") && typeof currentSectionIndex === "number") {
    lastSectionIndexBeforeRef = currentSectionIndex;
  }

  if (pushState) pushCurrentView();

  currentRefFile = refFile;
...
function goBack() {
  ...
  if (!prev) {
    if (typeof lastSectionIndexBeforeRef === "number") {
      showSection(lastSectionIndexBeforeRef, false);
      return;
    }
    showHome(false);
    return;
  ...
  if (prev.view === "ref" && prev.refFile) {
    showReference(prev.refFile, false);
    return;
  }

  if (prev.view === "section" && typeof prev.index === "number") {
    showSection(prev.index, false);
    return;
  }
...
function isNoDropdownSection(sectionTitle) {
  return NO_DROPDOWNS_SECTIONS.has(String(sectionTitle || "").trim());
}

function shouldNeverDropdown(labelText) {
  const t = String(labelText || "").trim().toLowerCase();
  return NEVER_DROPDOWN_LABELS.has(t);
}

function isKeywordDropdownTrigger(labelText) {
  const t = String(labelText || "").trim().toLowerCase();
  return KEYWORD_DROPDOWN_TRIGGERS.some(k => t.includes(k));
}

function isArrhythmiaDropdownTrigger(sectionTitle, labelText) {
  if (String(sectionTitle || "").trim() !== ARRHYTHMIAS_SECTION_TITLE) return false;
  const t = String(labelText || "").trim().toLowerCase();
  return ARRHYTHMIA_DROPDOWNS.some(x => t.startsWith(x));
}

function applyDropdownRules(root, sectionTitle) {
  if (isNoDropdownSection(sectionTitle)) return;

  root.querySelectorAll("li").forEach(li => {
    // already converted / manual dropdown
    if (li.querySelector(":scope > details.drop")) return;

    const nestedList = li.querySelector(":scope > ul");
    if (!nestedList) return;

    // label = text nodes only (exclude nested list text)
    ...
    if (!label) return;
    if (shouldNeverDropdown(label)) return;

    const labelLower = label.toLowerCase();
    const allowed =
      isKeywordDropdownTrigger(labelLower) ||
      isArrhythmiaDropdownTrigger(sectionTitle, labelLower);

    if (!allowed) return;

    // Build dropdown with button (existing styling)
    const panel = document.createElement("div");
    panel.className = "drop-panel";
    panel.appendChild(nestedList);
