...
function applyDropdownRules(root, sectionTitle) {
  ...
  // Agitation: dropdown only for specific antipsychotic agents (explicit user request)
  if (title.toLowerCase() === "agitation") {
    convertListHeadingsToDropdowns(root, AGITATION_AGENT_DROPDOWNS);
  }
}
...
function ensureFooterCredits() {
  ...
  p.className = "disclaimer subtle dev-credit";
  p.textContent = "Developed by Anna Gebhardt, DO and Brandon Zaffuto, MD.";
...
function pushCurrentView() {
  // Record the currently-visible view so "Back" returns to the correct place
  if (!els.refView.classList.contains("hidden")) {
    navStack.push({ view: "ref", refFile: els.refFrame?.getAttribute("src") || "" });
    return;
  }
  if (!els.sectionView.classList.contains("hidden")) {
